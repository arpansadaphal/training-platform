// Shared test helpers for packages/api router tests.
//
// Deliberately self-contained rather than importing from
// packages/db/src/repositories/tests/helpers.ts: reaching into another
// package's test directory would create a bad dev-time coupling, and the
// duplicated ~30 lines are cheaper to maintain than that coupling would be.
//
// Imports from @training/db's public surface are fine — that's the same
// surface the routers themselves consume.
//
// The cleanup walk is dependency-ordered to match the FK constraints in
// packages/db/prisma/schema.prisma. Most relations are ON DELETE RESTRICT,
// so children must be deleted before parents, and the circular
// Program ↔ ProgramVersion reference must be broken with an updateMany
// first. The walk covers every table a Program-owned test user could
// accumulate, not just what today's tests create.
//
// No $transaction wrapping: on remote Neon the whole walk routinely
// exceeds Prisma's 5s interactive-transaction timeout.

import { prisma, createUser, type UserRecord } from "@training/db";
...
export async function makeTestUser(
  ...
  const user = await createUser({
    email: `phase1-api-${slug}-${crypto.randomUUID()}@example.test`,
