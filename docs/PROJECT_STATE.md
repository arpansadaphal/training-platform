...
## Current phase
**Phase 2 (Analysis Engine) is next.** Phase 1 (Domain Model & Database) is complete. See `docs/phases/phase-02-*.md` and construct the handoff packet with `docs/HANDOFF_TEMPLATE.md`. Before starting Phase 2, complete the prerequisites listed under "Phase-1-specific outstanding items" below.
...
## Project conventions (as-built, learned during Phases 0–1)
- **Git**: each phase on its own branch `phase-NN-<short-name>`, merged to `main` via PR. Never commit phase work directly to `main`.
- **Node version**: pinned to 20 via `.nvmrc`. Node 22+ causes Vitest worker crashes ("Worker exited unexpectedly"). Always `nvm use 20` in a fresh shell before running pnpm.
- **Turbo env vars**: declared via `globalEnv` in `turbo.json`. Turbo 2.x strips undeclared env vars from task subprocesses — this bit us in CI where `DATABASE_URL` was set at the job level but invisible to `pnpm turbo run test`. Add any new runtime/build-time env var to `globalEnv`.
- **`turbo.json`'s `typecheck` task depends on `["^build", "build"]`** — deliberate. Prevents a race where `tsc` reads a half-regenerated `apps/web/.next/types/` directory when Turbo runs `build` and `typecheck` in parallel. Do not revert.
- **Imports in `packages/*`**: extensionless relative imports (`from "./router"`, not `"./router.js"`). Next.js webpack does not rewrite `.js` → `.ts` in `transpilePackages`.
- **Env files**: three locations — root `.env.local`, `apps/web/.env.local`, `packages/db/.env`. Next.js reads `apps/web/`; Prisma CLI reads `packages/db/`. Add new vars to all three for local dev.
- **Prisma on Vercel**: `packages/db/prisma/schema.prisma` declares `binaryTargets = ["native", "rhel-openssl-3.0.x"]`, and `apps/web/next.config.js` uses `@prisma/nextjs-monorepo-workaround-plugin`. Both required (ARCH-022).
- **Prisma `$transaction`**: passes explicit `{ timeout: 20000, maxWait: 10000 }` via exported `TRANSACTION_OPTIONS` in repository files. Remote Neon latency exceeds Prisma's 5s default.
- **`as Prisma.InputJsonValue` casts** in repository files are intentional boundary enforcement (ARCH-010). Do not "clean them up" by typing the input signature with Prisma types — that would leak Prisma into the repository's public API.
...
## Known bugs
None. Phase 1 migration `0002_domain_model` applied cleanly to local dev, CI Postgres, preview Neon branches, and production Neon branch.
...
- **Local Postgres for tests** — `packages/db` tests currently hit the remote `vercel-dev` Neon branch (ap-southeast-1), taking 3–4 min and occasionally failing on connection drops. Install Postgres locally (`brew install postgresql@16`) and point tests at it — suite drops to <10s. **Strongly recommended as Phase 2 pre-work.**
- **Sentry sourcemaps** — `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` set in Vercel; sourcemap uploads succeed during preview builds but not yet verified for production. Revisit when trace quality matters.
...
## Test status
- **Vitest**: 8 test files (up from 5). Phase 0: `packages/config` (3), `packages/domain` (1), `packages/ai` (1), `packages/db` (1), `packages/api` (2). Phase 1 adds: `packages/db` — `program.test.ts` (4 tests) + `programVersion.test.ts` (3 tests); `packages/api` — `program.test.ts` (3 tests, authorization).
  - **Locally**: `pnpm --filter @training/db test` runs against the `vercel-dev` Neon branch (slow — ~3–4 min, prone to connection flakiness).
  - **CI**: runs against an ephemeral `postgres:16` service container (fast, isolated, no remote dependency). See `.github/workflows/ci.yml`. **CI is the authoritative pass/fail signal**; local runs are for iterating on failures.
...
- **Database (production)**: Neon primary branch (`ep-quiet-sea-...`)
- **Database (dev/preview)**: `vercel-dev` Neon branch (`ep-flat-bread-...`); PR previews get isolated Neon branches via the Neon integration (installed during Phase 1)
- **Production schema**: `0002_domain_model` applied to production Neon branch via `prisma migrate deploy`. Production is on the same schema as dev.
- **Sentry**: receiving events from both client and server.
